...
public class WorldContainer : MonoBehaviour {
	...
	public GameObject PlayerNearTreeCheck(){
		return NearestObjectTo ("tree", player);
	}

	public GameObject NearestObjectToPlayer(string what) {
		return NearestObjectTo (what, player);
	}

	public GameObject NearestObjectTo (string what, GameObject target) {
	...
	public List<GameObject> GetAllNearbyObjects (string what, GameObject center, float radius) {
		List<GameObject> result = new List<GameObject> ();
		GameObject[] things;

		if (world_objects.TryGetValue (what, out things)) {
			foreach (GameObject thing in things) {
				float dist = Vector3.Distance (thing.transform.position, center.transform.position);
	...
	public void UpdateObjectArray(string what) {
		GameObject[] things;
